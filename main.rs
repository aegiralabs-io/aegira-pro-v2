...
fn process_incident(
    ...
    if action == "dry_run" {
        log_incident(&format!("[DRY RUN] Rule '{}' matched. Remediation was NOT executed.", rule.id));
        log_incident(&format!("[DRY RUN] Would execute: {}", describe_remediation(&rule.remediation)));
        send_alert(
            &format!("Aegira: {} dry run", rule.name),
            &alert_body(incident, Some(rule), "DRY RUN - REMEDIATION NOT EXECUTED"),
        );
        log_incident(&format!("[DRY RUN] Incident handled in {:.2?}", start.elapsed()));
        return;
    }
    ...
    if action == "approval_required" {
        ...
        send_alert(
            ...
            &alert_body(incident, Some(rule), "APPROVAL REQUIRED - REMEDIATION NOT EXECUTED"),
        );
        return;
    }
    ...
    match recover_with_rule(rule) {
        Ok(()) => {
            ...
            if load_config().map(|c| c.alerts.notify_on_recovery).unwrap_or(false) {
                send_alert(
